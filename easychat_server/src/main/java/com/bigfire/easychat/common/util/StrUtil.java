...
//    }
    ...
    public static void print(Object object)
    {
        ...
        if(System.getProperties().getProperty("os.name").toLowerCase().contains("linux")){//linux
     ...
     * <pre>
     *  StrUtil.debug("123");
     *  linux环境下 = [main(Test.java:13)]123
     *  非linux下 = [com.test.util.Test.main(Test.java:13)]123
     *  其中如果是在编辑器中可以直接点击Test.java:13跳转到制定行 快速定位
     * </pre>
     *
     * @param obj 要打印的对象
     * @since 3.1.1
    ...
    {
        StackTraceElement ste = new Throwable().getStackTrace()[1];
        StringBuilder sb = new StringBuilder().append(ste.getMethodName()).append("(").append(ste.getFileName()).append(":").append(ste.getLineNumber()).append(")");
        if(System.getProperties().getProperty("os.name").toLowerCase().contains("linux")){//linux环境认为是线上运行环境
            out.println("["+sb.toString()+"]"+obj);
        }else {
            out.println("["+ste.getClassName()+"." + sb.toString()+"]"+obj);
        }
    }
